...
const CampaignDetails = () => {
  ...
  const { donateToCampaign, sendTransactionsLoading } = useStateContext();

  const [amount, setAmount] = useState('');

  const { data: contractDonators } = useReadContract({
    contract,
    method: 'function getDonators(uint256 _id) view returns (address[], uint256[])',
    params: [state.pId],
  });

  const donators = useMemo<any[]>(() => {
    if (!contractDonators) {
      return [];
    }

    const numberOfDonations = contractDonators[0].length;
    const parsedDonations = [];

    for (let i = 0; i < numberOfDonations; i++) {
      parsedDonations.push({
        donator: contractDonators[0][i],
        donation: ethers.formatEther(contractDonators[1][i].toString()),
      });
    }
  ...
  return (
    <div>
      {sendTransactionsLoading && <Loader/>}
